function [temp2]  = TOPTREEBzcmdLine(I_lat, I_lon, I_tilt,I_haf, I_mgram, I_cycle, I_tempCyc, I_enlilBB, I_enlilVV, I_AT, varargin)
% (I_lat, I_lon, I_tilt,I_haf, I_mgram I_cycle I_tempCyc I_enlilBB I_enlilVV I_AT
% -15.0 14.0 30.0 58.0 South 24 1 13 645 20161230223000
% [temp2]  = TOPTREEBzcmdLine('-15.0', '14.0', '30.0', '58.0', 'South', '24', '1', '13', '645', '20161230223000');
% ./run_TOPTREEBzcmdLine.sh /Applications/MATLAB/MATLAB_Runtime/v93 -15.0 14.0 30.0 58.0 South 24 1 13 645 20161230223000
% 
% 
...
%% converts cmd Line input strings to structure of input Vars.
Hinput.so = [str2num(I_lat),str2num(I_lon)];
Hinput.I_tilt = str2num(I_tilt);
Hinput.I_haf = str2num(I_haf);
Hinput.I_Mgram =I_mgram; 
Hinput.cycle = str2num(I_cycle);
Hinput.tempCyc = str2num(I_tempCyc);  % only 1 or -1
Hinput.enlilBB = str2num(I_enlilBB);
Hinput.enlilVV = str2num(I_enlilVV);
Hinput.AT=datenum([str2num(I_AT(1:4)),str2num(I_AT(5:6)),str2num(I_AT(7:8)),...
                str2num(I_AT(9:10)),str2num(I_AT(11:12)),str2num(I_AT(13:14))]);
...
%% REPLICATE TOPTREEBzTool
inputString.Hinput = Hinput;
