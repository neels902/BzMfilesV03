function [ OUTPUT ] = ReadOMNIVec(IN_time)
...
if arrTime <= datenum([2005,3,0,0,0,0])
    UrlStart= 'http://spdf.gsfc.nasa.gov/pub/data/ace/merged/4_min_merged_mag_plasma/ace_m';
    AceBegTime=arrTime - datenum([0,8,0,00,00,00]);  % reads data prior to arrival Time
    AceEndTime=arrTime + datenum([0,8,0,00,00,00]);
    ...
    year=ArrTime2 .* ones(length(Bmag),1);
    doy=data{1,2};
    ...
    loga= t>AceBegTime && t<AceEndTime ;
    t=t(loga,:);
    ...
    [OUTPUT] = ReadJsonVec(UrlStart, IN_time);
end
